...
string generate_map_header_text(Json map_data, Json layouts_data) {
    ...
    text << mapName << ":\n"
         << "\thost_ptr " << json_to_string(layout, "name") << "\n";

    if (map_data.object_items().find("shared_events_map") != map_data.object_items().end())
        text << "\thost_ptr " << json_to_string(map_data, "shared_events_map") << "_MapEvents\n";
    else
        text << "\thost_ptr " << mapName << "_MapEvents\n";

    if (map_data.object_items().find("shared_scripts_map") != map_data.object_items().end())
        text << "\thost_ptr " << json_to_string(map_data, "shared_scripts_map") << "_MapScripts\n";
    else
        text << "\thost_ptr " << mapName << "_MapScripts\n";
    ...
    if (map_data.object_items().find("connections") != map_data.object_items().end()
     && map_data["connections"].array_items().size() > 0 && json_to_string(map_data, "connections_no_include", true) != "TRUE")
        text << "\thost_ptr " << mapName << "_MapConnections\n";
    else
        text << "\thost_ptr NULL\n";
...
string generate_map_connections_text(Json map_data) {
    ...
    text << "\n" << mapName << "_MapConnections:\n"
         ...
         << "\t.if LINUX64\n\t.space 4\n\t.endif\n"
         << "\thost_ptr " << mapName << "_MapConnectionsList\n\n";
...
string generate_layout_headers_text(Json layouts_data) {
    ...
    for (auto &layout : layouts_data["layouts"].array_items()) {
        ...
        text << border_label << "::\n"
             ...
             << "\thost_ptr " << border_label << "\n"
             << "\thost_ptr " << blockdata_label << "\n"
             << "\thost_ptr " << json_to_string(layout, "primary_tileset") << "\n"
             << "\thost_ptr " << json_to_string(layout, "secondary_tileset") << "\n";
        if (version == "firered") {
