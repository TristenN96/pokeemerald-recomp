...
static TaskFunc Swap_GetFollowUpTaskFunc(u8 taskId)
{
    TaskFunc func;
    GbaAddr funcAddr = ((u16)gTasks[taskId].tFollowUpTaskPtrHi << 16)
                     | (u16)gTasks[taskId].tFollowUpTaskPtrLo;
    HostResolveFunction(funcAddr, &func, sizeof(func));
    return func;
}

#define SET_SWAP_FOLLOWUP(taskId, function) \
    do { \
        GbaAddr funcAddr = HOST_FUNCTION_ADDR(function); \
        gTasks[taskId].tFollowUpTaskPtrHi = funcAddr >> 16; \
        gTasks[taskId].tFollowUpTaskPtrLo = funcAddr; \
    } while (0)
    ...
    case STATE_YESNO_HANDLE_INPUT:
        if (JOY_NEW(A_BUTTON))
        {
            ...
            if (sFactorySwapScreen->yesNoCursorPos == 0)
            {
                ...
                gTasks[taskId].func = Swap_GetFollowUpTaskFunc(taskId);
            }
            else
            {
                // Selected No
...
static void Swap_HandleQuitSwappingResponse(u8 taskId)
{
    ...
    {
        gTasks[taskId].tState = 0;
        SET_SWAP_FOLLOWUP(taskId, Swap_Task_HandleChooseMons);
...
static void Swap_AskQuitSwapping(u8 taskId)
{
    if (gTasks[taskId].tState == 0)
    {
        ...
        SET_SWAP_FOLLOWUP(taskId, Swap_HandleQuitSwappingResponse);
...
static void Swap_AskAcceptMon(u8 taskId)
{
    if (gTasks[taskId].tState == 0)
    {
        ...
        gTasks[taskId].tState = STATE_YESNO_SHOW;
        SET_SWAP_FOLLOWUP(taskId, Swap_HandleAcceptMonResponse);
        gTasks[taskId].func = Swap_Task_HandleYesNo;
    }
}
...
static void Swap_Task_HandleChooseMons(u8 taskId)
{
    ...
    case STATE_CHOOSE_MONS_HANDLE_INPUT:
        ...
        else if (JOY_NEW(B_BUTTON))
        {
            ...
            Swap_PrintMonSpeciesAtFade();
            Swap_EraseSpeciesWindow();
            SET_SWAP_FOLLOWUP(taskId, Swap_AskQuitSwapping);
            gTasks[taskId].tState = 0;
...
{
    SET_SWAP_FOLLOWUP(taskId, Swap_Task_SwitchPartyScreen);
    gTasks[taskId].tFollowUpTaskState = 0;
    gTasks[taskId].tState = 0;
    gTasks[taskId].func = Swap_Task_ScreenInfoTransitionOut;
}
...
static void Swap_ActionMon(u8 taskId)
{
    if (!sFactorySwapScreen->inEnemyScreen)
    {
        SET_SWAP_FOLLOWUP(taskId, Swap_Task_HandleMenu);
    ...
    {
        SET_SWAP_FOLLOWUP(taskId, Swap_AskAcceptMon);
        gTasks[taskId].tFollowUpTaskState = 0;
    }
    gTasks[taskId].tState = 0;
    gTasks[taskId].func = Swap_Task_ScreenInfoTransitionOut;
