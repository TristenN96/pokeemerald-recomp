...
static void CB2_SaveFailedScreen(void)
{
    ...
    default:
        ...
        SetWindowAttribute(sWindowIds[TEXT_WIN_ID], 7, HostPointerToGbaAddr(&gDecompressionBuffer[0x2800]));
        sWindowIds[CLOCK_WIN_ID] = AddWindowWithoutTileMap(sWindowTemplate_Clock);
        SetWindowAttribute(sWindowIds[CLOCK_WIN_ID], 7, HostPointerToGbaAddr(&gDecompressionBuffer[0x3D00]));
