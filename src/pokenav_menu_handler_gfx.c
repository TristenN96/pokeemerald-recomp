...
static void CreateBgDotPurplePalTask(void)
{
    ...
    SetWordTaskArg(taskId, 1, HostPointerToGbaAddr(sPokenavBgDotsPal + 1));
    SetWordTaskArg(taskId, 3, HostPointerToGbaAddr(sPokenavBgDotsPal + 7));
}
...
{
    u8 taskId = CreateTask(Task_UpdateBgDotsPalette, 3);
    SetWordTaskArg(taskId, 1, HostPointerToGbaAddr(sPokenavBgDotsPal + 7));
    SetWordTaskArg(taskId, 3, HostPointerToGbaAddr(sPokenavBgDotsPal + 1));
}
...
static void Task_UpdateBgDotsPalette(u8 taskId)
{
    u16 sp8[2];
    s16 *data = gTasks[taskId].data;
    const u16 *pal1 = (const u16 *)HostResolveGbaAddr(GetWordTaskArg(taskId, 1));
    const u16 *pal2 = (const u16 *)HostResolveGbaAddr(GetWordTaskArg(taskId, 3));
